...
describe("task-modify service", () => {
    ...
    describe("stop task", () => {
        ...
        it("success if the task is in running.", async () => {
            ...
            const result = await stopTask.execute(account, id, stopTime);
        ...
        it("throw if the account is not allowed for the task.", async () => {
            const stopTask = new StopTask(repo);
            const time = new Date();
            await expect(stopTask.execute("not allowed account", id, time)).rejects.toThrow(NotAllowed);
        });
    });
    ...
    describe("resume task", () => {
        ...
        it("success if the task is stopped.", async () => {
            ...
            await startTask.execute(account, id, startTime);
            await stopTask.execute(account, id, stopTime);
            const result = await resumeTask.execute(account, id, resumeTime);
        ...
        it("throw if the account is not allowed for the task.", async () => {
            const resumeTask = new ResumeTask(repo);
            const time = new Date();
            await expect(resumeTask.execute("not allowed account", id, time)).rejects.toThrow(NotAllowed);
        });
    });
